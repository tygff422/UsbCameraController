"""UsbCameraControllerの実機不要な純粋ロジック部分を検証する単体テスト。

open/capture/release等、実際にOpenCVでカメラデバイスを触る部分は実機が必要なため
test_usb_camera_controller.py（手動実行スクリプト）で確認する。ここでは、ファイルパス
...
def test_make_img_path_uses_custom_img_dir(tmp_path):
    controller = UsbCameraController(img_dir=tmp_path)
...
def test_make_img_path_creates_directory_if_missing(tmp_path):
    ...
    controller = UsbCameraController(img_dir=img_dir)
...
def test_is_led_on_returns_false_when_camera_not_opened():
    ...
    controller = UsbCameraController()
